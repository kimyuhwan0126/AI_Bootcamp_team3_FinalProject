...
console.log('\n[탐4-2] 홈이 옛 판 모양이다');
{
  ...
  /* 옛 판은 셋(＋·→·🔔)이었지만 알림은 없는 기능이라 안 그리고(논의105),
     → 도 오늘 없앴다 — 초대는 실제 링크로 바로 들어오는 길이 전부라 홈에서 누를
     '참가' 단추 자체가 갈 곳이 없다. 이제 로고 + 만들기(＋) 단추 하나뿐이다 */
  const 링크수 = await pg.locator('[data-slot="홈머리말"] a').count();
  ok('머리말이 로고 + 동그란 단추 하나다', 링크수 === 2, `${링크수}개`);
...
console.log('\n[탐7] ★ [이 출발지들로 모임 만들기] → /new 에 출발지가 들어가 있다');
{
  ...
  ok('두고 온 곳이 몇인지 말해 준다', /나머지 1곳은 친구들이 들어와서 각자 넣어요/.test(t), t.slice(0, 200));
  ok('이동 수단도 따라왔다',
