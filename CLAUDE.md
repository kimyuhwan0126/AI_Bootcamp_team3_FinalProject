...
- 브랜치: `main`(배포) ← `develop`(통합) ← `feat/*`, `fix/*`
- **파일 수정 시 반드시 `CHANGELOG.md` 변경기록 표에 행을 추가한다**
- 비밀키는 절대 커밋하지 않는다 (`.env*.local` 은 `.gitignore` 에 있음)

### 커밋 접두사 = 버전 규칙 (Conventional Commits)

| 접두사 | 뜻 | 버전 영향 |
|---|---|---|
| `feat:` | 기능 추가 | MINOR (`0.2.0` → `0.3.0`) |
| `fix:` | 버그 수정 | PATCH (`0.3.0` → `0.3.1`) |
| `refactor:` | 동작 그대로, 구조만 | 없음 |
| `docs:` `test:` `chore:` | 문서 · 테스트 · 잡일 | 없음 |

예: `feat: 모임 생성 API 추가` (본문 한글 OK)

### 버전 (SemVer)

- **최종 발표일 = `1.0.0`.** 그전까지는 전부 `0.y.z` (= "정식 출시 전 개발 버전")
- 버전은 **`main` 에서 통합 담당자만** 올린다 (`npm version minor` → 커밋+태그 자동)
- `1.0.0` 은 발표일에 **딱 한 번**. 그전에 실수로 올리지 않는다
- 상세: `docs/버전관리.md`

> ⚠️ 옛 저장소의 `v8` 은 "8번째 시제품"이라는 뜻이고, 새 저장소의 `0.1.0` 은
> "정식 출시 전"이라는 뜻이다. **숫자가 내려간 게 아니라 의미가 다른 체계**다.
