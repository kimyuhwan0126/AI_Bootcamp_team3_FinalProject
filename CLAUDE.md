...
>
> ## ⭐ 지금 무엇을 만드는가 — `docs/설계_v19.md`
>
> **2026-08-09 부터 이 저장소의 구현 기준은 [`docs/설계_v19.md`](docs/설계_v19.md) 다.**
> 그릴링으로 확정한 v19 스펙(화면 11대 · 상태기계 · 확정 범위 분기 · 단계 규칙 ·
> reopen 사다리 · 역할 권한 · AI 정책)이 들어 있고, 순서도 원본은 `docs/설계도/` 5개다.
>
> - **이 파일은 *어떻게* 만드는가**(폴백·데이터 계층·색상·커밋 규칙)를 정한다
> - **`설계_v19.md` 는 *무엇을* 만드는가**를 정한다
> - 아래 §0~§3 의 화면·기능 서술이 `설계_v19.md` 와 다르면 **`설계_v19.md` 가 기준**이다
>   (예: '가게' → **'지점'**, 선입금 → **'지점 카카오 링크 연결'** 로 개칭됨)
>
> 발표는 **로컬 실행 + 팀원 각 기기 IP 접속**이다 → `docs/발표_로컬시연.md` · `npm run dev:lan`
>
...
| 문서 | 내용 |
|---|---|
| **`docs/설계_v19.md`** | ⭐⭐ **구현 기준** — 무엇을 만드는가 (화면 11대·분기·규칙·권한·AI) |
| **`docs/설계도/`** | v19 순서도 원본 HTML 5종 (브라우저로 열면 확대·이동 됨) |
| **`docs/발표_로컬시연.md`** | 발표 당일 — 로컬 실행 + 팀원 기기 IP 접속 (카카오 등록·방화벽·대비책) |
