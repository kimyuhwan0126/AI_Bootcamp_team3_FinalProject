...
`MeetingClient.tsx` 905줄. 화면 조각은 전부 `sections/` 로 나왔고,
**`sections/` 안은 전부 400줄 미만**이다(최대 `LeaderBar.tsx` 173줄).

⚠️ **다만 저장소 전체로는 아직 400줄 넘는 파일이 남아 있다** — 이 화면 밖이다:

| 파일 | 줄 | 누가 만지나 |
|---|---|---|
| `app/page.tsx` | 1,118 | 홈 화면. 중간지점·투표·지도가 한 파일에 (투표·추천 담당 영역) |
| `lib/store.ts` | 657 | 🔒 통합 담당자 |
| `lib/ai.ts` | 590 | 👤 채팅·AI 파싱 담당 |

`app/page.tsx` 와 `lib/ai.ts` 는 **팀원 소유인데 400줄을 넘는다.** 담당자가 붙기 전에
쪼개는 것이 좋다.
