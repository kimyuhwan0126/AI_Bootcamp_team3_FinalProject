...
export default function 탐색() {
  ...
  /* 핀 이름표는 "가운데" 같은 우리 말이 아니라 그 자리의 실제 지명을
     ...
     까닭이 없다(왕복 하나 아끼는 것 + AI 가 실제로 추천한 이름과 어긋나지 않게).
  ...
  return (
    <section className={s.칸} data-slot="탐색" data-출발지수={출발지들.length} data-기준={기준}
      ...
      {/* 가운데를 잡는 기준 — 거리·AI (사용자 요청, 2026-08-17 · 시간 기준은 2026-08-23 에
          ...
          ⚠ 2026-08-23 — "가운데를 잡는 기준" 이름표(글자)는 없앴다, 사용자 요청 —
          거리·AI 두 단추만 봐도 무엇을 고르는 자리인지 알 수 있다는 판단. data-slot 은
          시험이 그대로 붙잡는다(이름표가 없어도 이 칸 자체는 남아 있다). */}
