...
/* 자리 이름. 차례가 곧 '위로 한 칸' 순서다(미니가 가장 낮고 가득이 가장 높다).
   ...
   ⚠ 목업에는 미니와 반 사이에 **'살짝'(남는 높이의 30%)** 이 하나 더 있었다 —
   2026-08-19 에 사람이 정해 뺐다. 손잡이를 눌러 도는 자리가 넷에서 셋으로 줄어,
   요약만 보다가 한 번 누르면 바로 반이 열린다. 되살릴 일이 생기면 사람에게 먼저 묻는다
   (셈은 lib/시트스냅.ts 에 있었다). */
export const 자리들 = ['숨김', '미니', '반', '가득'] as const;
export type 자리 = (typeof 자리들)[number];

/* 보여 줄 것이 있으면 세 자리가 살아난다. 없으면 '숨김' 하나뿐이라 끌어도 안 움직인다.
   (목업 snapOrder 1689–1692 와 같은 규칙이되, 가르는 잣대가 다르다 — 아래 머리말 참고.) */
const 펼친자리: 자리[] = ['미니', '반', '가득'];
 ...
 * 사라진다.** 그런데 지금 시트 안에는 만들기 단추와 AI 추천이 들어 있다 —
 * 예전 홈에서는 스크롤만 하면 늘 닿던 것들이다.
...
export function use시트(펼칠수있나: boolean) {
  ...
  const 스냅 = useRef<Record<자리, number>>({ 숨김: 0, 미니: 0, 반: 0, 가득: 0 });
  const 치우침 = useRef(0);
