...
import { 갈래고르기, 아이콘고르기, 색고르기, 갈래들, type 갈래 as 갈래이름 } from '@/lib/장소갈래';
...
type 장소 = {
  ...
  /** 카카오가 주는 가게 전화. 없는 가게가 많다 — 있을 때만 시트에 선다. */
  phone?: string;
  /** 카카오 장소 페이지 주소. 사진·후기는 우리가 안 받아 오므로 그리로 가는 길 하나를 둔다. */
  url?: string;
};
...
/* 시트 요약 줄의 '빼기' 그림. 글자('삭제')가 아니라 아이콘인 까닭은 그 줄이 한 줄이라
   ...
   ⚠ 2026-08-20 — 휴지통에서 **✕ 로 바꾸고 키웠다**(사람이 정했다). 같은 날 오전에
   출발지 칩의 ✕ 가 사라지면서 이것이 **삭제로 가는 하나뿐인 길**이 됐는데, 17px 짜리
   휴지통은 그 무게가 아니었다. ✕ 는 칩에서 없어진 그 그림이기도 해서, 하던 손짓이
   자리만 옮겨 이어진다. */
const 빼기표 = (
...
/* 이동 수단은 **출발지마다** 하나씩이다(2026-08-19 사용자 요청) — 차를 가져오는 사람과
  ...
  /* 칩 3단 탭(목업 chipTap 3603–3623 — 지도 이동 → 시트 열기 → 시트 내리기)은
     2026-08-20 에 사람이 정해 뺐다. 같은 단추가 누를 때마다 다른 일을 하면 무엇이
     일어날지 누르기 전에 알 수 없었기 때문이다.
     같은 날 오후, 칩이 하는 일이 **하나에서 둘로** 늘었다(사람이 정했다): 지도를 그
     자리로 옮기고 + 그 출발지 시트를 '반'까지 세운다. 3단 순환과 다른 점은 **몇 번을
     눌러도 결과가 같다**는 것이다 — 아래 `칩누름` 을 본다. */
  /* 시트가 무엇을 보여 주는가 — 가운데인가, 어느 출발지인가, 둘레의 어느 장소인가.
     '장소' 는 2026-08-20 에 늘었다(사용자 요청): 탐색 모드에서 지도 표식을 누르면 그
     장소의 시트가 올라온다. 무엇을 눌렀는지는 `고른장소`(아래)가 함께 쥔다 —
     지도 표식이 커지는 것도 그 값을 본다.

     ⚠ 둘은 **늘 함께 움직인다** — 가운데 말풍선이나 출발지 핀으로 옮겨 가면 `고른장소`
     도 함께 푼다(아래 `가운데누름`·`출발지핀누름`·`칩누름`). 안 풀면 지도에서는 장소
     표식이 커진 채인데 시트는 딴 것을 말해, 무엇을 고른 것인지 화면이 두 가지로 답한다. */
  const [고른것, set고른것] = useState<
    { 갈래: '가운데' } | { 갈래: '출발지'; i: number } | { 갈래: '장소'; id: string }
  >({ 갈래: '가운데' });
  ...
  const 지도장소들 = useMemo(
    () => 보일장소들.map((p) => ({
      ...
      갈래: 갈래고르기(p.category, p.categoryDetail),
    })),
    [보일장소들],
  );

  /* 지도 표식을 누르면 — 그 장소의 시트가 올라온다(2026-08-20 사용자 요청).
     ⚠ 전에는 쪽지 한 줄('이름 · 주소')을 띄우는 것이 전부였다. **그 쪽지는 뺐다** —
     시트가 같은 이름과 주소를 더 크게, 사라지지 않는 자리에 적는다. 남겨 두면 1.8초
     동안 같은 말이 화면에 두 번 있게 된다. */
  const 장소누름 = useCallback((id: string) => {
    ...
    set고른것({ 갈래: '장소', id });
    set모드('상세');
    /* 이미 '상세' 면 모드가 안 바뀌어 시트 자리를 세우는 효과가 안 돈다 — 손으로 내려
       둔 시트도 다시 올라와야 하므로 여기서 직접 세운다(칩누름과 같은 규칙). */
    시트참.current.가기('반', true);
  }, [장소들]);

  /* 고른 장소가 지도에서 사라지면 시트도 가운데로 돌아간다 — 갈래 칩을 갈아탔거나,
     탐색을 껐거나, 가운데가 움직여 둘레를 다시 받아 온 때다. 그 셋을 각각 손보는 대신
     **`고른장소` 한 값만 본다**: 어느 길로 풀렸든 여기 한 곳에서 걸린다. */
  useEffect(() => {
    set고른것((전) => (전.갈래 === '장소' && 전.id !== 고른장소 ? { 갈래: '가운데' } : 전));
  }, [고른장소]);
  ...
  /* 지도 빈 곳 누르기 — 자리를 한 칸씩 돈다(상세 → browse → clean → browse).

     ⚠ **커진 장소 표식이 있으면 그것부터 접는다**(2026-08-20 사용자 요청). 물방울로
     커진 핀과 그 장소 시트는 한 벌이라 함께 풀려야 한다 — 지도를 눌러 시트만 내려가고
     핀은 커진 채로 남으면, 지도는 아직 그 가게를 고른 것처럼 말한다.
     `고른장소` 를 풀면 위 효과가 시트를 가운데로 되돌리고, 'browse' 가 그 시트를
     요약 한 줄(미니)까지 내린다 — 곧 눌러서 열기 전의 자리다. */
  const 지도누름 = useCallback(() => {
    if (고른장소) { set고른장소(null); set모드('browse'); return; }
    set모드((m) => (m === '상세' ? 'browse' : m === 'browse' ? 'clean' : 'browse'));
  }, [고른장소]);

  /* 말풍선을 누르면 상세로 — 시트를 어디에 세울지는 위 효과가 맡는다.
     둘레 장소를 켜 뒀으면 그것도 함께 푼다(위 `고른것` 머리말 — 한 번에 하나만 켜진다). */
  ...
  /* 칩을 누르면 — 지도를 그 출발지로 옮기고 **그 출발지 시트를 '반'까지 올린다**
     (2026-08-20 사용자 요청). 그날 아침까지는 지도만 옮겼고 자세한 것은 지도 핀으로만
     열 수 있었는데, 핀은 지도가 죽으면 사라지고 겹친 핀은 누르기도 어렵다 — 늘 같은
     자리에 서 있는 칩이 같은 곳으로 가는 길이라야 한다.

     ⚠ 모드를 '상세'로 올리는 것이 곧 **탭바를 내리는 일**이다(아래 `data-홈모드` 효과).
     안 내리면 올라온 시트 밑단이 탭바(z-index 10, 셸보다 위)에 깔린다.

     ⚠ 이미 '상세'면 모드가 안 바뀌어 시트 자리를 세우는 효과가 안 돈다 — 그때는
     여기서 직접 올린다. 손으로 '미니'까지 내려 둔 뒤 칩을 눌러도 다시 반이 되어야 한다.

     ⚠ 2026-08-20 오후 — **같은 칩을 다시 누르면 시트가 내려간다**(사람이 정했다).
     열 때와 닫을 때가 같은 자리라야 한다: 그 전에는 여는 길은 칩인데 닫는 길은
     손잡이나 지도 탭이라, 칩을 다시 눌러 본 사람은 아무 일도 안 일어난다고 느꼈다.
     3단 순환과는 다르다 — 도는 자리가 둘(열림 ⇄ 닫힘)이라 다음에 무엇이 될지
     지금 화면만 보면 안다. **'그 칩이 이미 열려 있을 때'만** 닫는다: 다른 칩을 누르면
     늘 그 출발지로 갈아탄다. */
  const 칩누름 = useCallback((i: number) => {
    const o = 출발지들[i];
    if (!o) return;
    /* 이미 이 출발지의 시트가 올라와 있으면 — 내린다. 지도는 안 건드린다:
       보고 있던 자리를 그대로 두는 것이 '시트만 접었다'는 뜻에 맞는다. */
    if (모드 === '상세' && 고른것.갈래 === '출발지' && 고른것.i === i) {
      set모드('browse');
      return;
    }
    set초점(i);
    ...
    set고른장소(null);
    set고른것({ 갈래: '출발지', i });
    if (모드 === '상세') 시트참.current.가기('반', true);
    else set모드('상세');
  }, [출발지들, 모드, 고른것]);
  ...
  /* 지금 시트가 말하는 둘레 장소. 목록에서 사라졌으면 null 이고, 그때는 위 효과가
     곧 시트를 가운데로 되돌린다 — 여기서는 한 번 비는 그림만 안 그리면 된다. */
  const 시트장소 = 고른것.갈래 === '장소' ? (장소들.find((p) => p.id === 고른것.id) ?? null) : null;
  const 시트장소갈래 = 시트장소 ? 갈래고르기(시트장소.category, 시트장소.categoryDetail) : null;
      ...
      <div ref={시트.시트칸} className={s.시트} data-slot="시트" data-스냅={시트.지금자리}>
        ...
        <div ref={시트.요약칸} className={s.시트요약} data-slot="시트요약">
          {고른것.갈래 === '출발지' && 출발지들[고른것.i] ? (
            ...
            /* 둘레 장소 — 지도 표식과 **같은 그림·같은 색**이라야 '저 핀이 이 시트'가 읽힌다.
               색은 갈래마다 다르다(lib/장소갈래.ts 의 `갈래색`) — 출발지 갈래가 바로 위에서
               `출발지색고르기` 로 같은 일을 한다.
               갈래를 모르는 곳은 갈래 자리를 비운다: '기타' 같은 말을 지어내지 않는다. */
            <>
              <span className={s.그림} style={{ ['--c' as string]: 색고르기(시트장소갈래) }} aria-hidden>
                <갈래아이콘 아이콘={아이콘고르기(시트장소갈래)} 크기={17} />
              </span>
              <span className={s.이름}>{시트장소.name}</span>
              {시트장소갈래 && <span className={s.요약값}>{시트장소갈래}</span>}
            </>
          ) : (
            <>
