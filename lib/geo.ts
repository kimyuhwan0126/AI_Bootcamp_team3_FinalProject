...
/* 받아 주는 자리는 한국으로 못 박는다 (그릴링 결정). 제주·울릉·독도가 들어오는 상자다.
   ...
   ⚠ 상자 자체는 `lib/한국상자.ts` 로 옮겼다 — 이 파일은 맨 위에서 곧바로 neon() 을 만들어
   브라우저가 못 부르는데, 화면(`app/탐색/탐색.tsx`)도 같은 상자가 필요해 손으로 베껴 두고
   있었다. 여기서 다시 내보내니 이 파일을 부르던 자리는 하나도 안 바꿔도 된다. */
export { 한국안 } from './한국상자';
