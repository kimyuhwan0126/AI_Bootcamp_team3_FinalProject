...
const 길별한도: Record<string, { 창: number; 한도: number }> = {
  ...
  /* 홈 탭 AI 추천(2026-08-22) — geo·places 와 달리 이건 **진짜 토큰이 나간다**(lib/ai.ts).
     ...
     사람이 손으로 눌러야 부르는 자리(opt-in)이니 짧은 시간에 5번이면 정상 사용은 안
     걸리고, 연타·긁어 가기만 걸린다.
     ⚠ 2026-08-23 — 처음엔 10분에 5번으로 뒀는데, 실사용(직접 테스트)해 보니 다른
     권역을 몇 개 이어서 재보는 것 같은 정상적인 반복 사용도 5번을 빨리 채워서
     한참(최대 10분) 기다려야 했다. reasoning_effort:'none' 으로 호출 하나가
     비싸지 않아졌으니(응답도 1~2초로 짧다) 창을 10분→1분으로 좁힌다 — **한도(5번)는
     그대로**라 촘촘한 연타는 여전히 걸리고, 그냥 창만 짧아져 정상적으로 몇 번 더
     눌러 보는 사람은 덜 기다린다. */
  'home-ai': { 창: 60_000, 한도: 5 },
};
