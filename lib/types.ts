...
/**
 * 후보 상한 — **지역·지점 모두 5개** (2차 그릴링, 2026-08-10 채택).
 *
 * ⚠️ v19 는 "지점 상한 없음"이었다(v7 이 5개 제한을 철회). 2차 그릴링이 그걸
 *    되돌렸고 근거가 명확하다: **"여섯 개를 놓고 고르라고 하면 표가 흩어져
 *    1위가 안 나온다."** 4명이 6개 후보에 찍으면 1표씩 갈려 동점 천지가 된다.
 *
 * 세는 방식
 *   · **추천 후보도 포함**한다 — AI 가 다섯 자리를 먼저 차지해서는 안 된다
 *   · **병합은 새 후보가 아니다** — 같은 동에 여러 명이 찍는 건 상한과 무관하다
 *   · 상한에 걸리면 **지우고 다시 등록**하면 된다(등록 단계에는 삭제권이 있다)
 */
