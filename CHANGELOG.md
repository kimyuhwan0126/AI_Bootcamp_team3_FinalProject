...
|---|---|---|---|---|
| 2026-08-06 | Claude(AI) | `docs/팀_개발환경.md` | **§5-1 "눈으로 확인하기 — 실제 응답을 꺼내는 법" 신설.** ① `/api/debug` 로 모임을 시드(`ENABLE_DEBUG=1` 필요, 없으면 운영 빌드에서 403)한 뒤 `?code=` 로 조회하는 절차 ② **`?action=…` 은 GET 에 없다**는 경고(`app/api/meeting/route.ts` 의 GET 은 `code` 만 받는다) ③ 서버 정리는 프로세스명이 **`next-server`** 라 `grep "[n]ext-server"` 로 잡아 `kill -9` ④ 기존 `pkill -f` 금지 경고 유지 ⑤ 응답이 맞아도 화면은 별개이므로 `/m/{코드}` 를 열어보라는 항목 | PR #55 브랜치 세션이 검증 절차에서 **두 번 헛디뎠다.** ① `?action=places&region=…` 로 확인하려다 **빈 응답**을 받았다 — 값이 0 이라서가 아니라 **애초에 아무것도 안 돌려준 것**이라, 눈치 못 챘으면 "확인했다"고 착각한 채 넘어간다. ② `ps aux \| grep "next start"` 로 서버를 못 잡았다 — 실제 프로세스명이 `next-server` 라 **조용히 0건**이 나오고 정리된 줄 알게 된다(포트가 물린 채 다음 검증이 옛 빌드를 보게 된다). 둘 다 **실패가 눈에 안 띄는 종류**라 문서에 박아둔다. `pkill -f "next start"` 가 셸을 죽인다는 기존 경고는 정확했다 — 브랜치 세션이 무심코 썼다가 그대로 재현했다 |
...
| 2026-08-06 | Claude(AI) | `memory/status.md` | **PR #48~#54 반영.** ① ⚡스냅샷을 `develop a5d75a5`(main 대비 44커밋)로 갱신 + 중간지점 하드코딩 문제 해결 행 추가 ② **PR #54 절 신설** — 팀원 `feat/ai-vote-sheets` 사전검증 결과(💰 0콜, 전 항목 통과, CI 4/4 green, `mergeable_state: clean`)와 **답변 대기 3건**(`playwright-core` 미등재 · Vercel 서버리스 Chrome + `maxDuration=300` · 비공식 엔드포인트) ③ **PR #48~#53 절 신설** — CEO Preview 지적 6건 반영 이력과 **교훈 3가지**(호출부 확인 · 브랜치는 `origin/develop` 에서 · 고정 임계값은 지역마다 다르게 망가진다) ④ 미해결 3건 명시(`거리순`↔`시간순` 이 키 없이는 동일 · `일부 거리 추정` 문구 보류 · 카카오맵 링크 클릭 미검증) | 코드 변경 없는 **문서 갱신**이다. status.md 는 새 세션이 가장 먼저 읽는 파일인데 PR #46 시점(`d76c48b`, 37커밋)에 멈춰 있어, 다음 세션이 **7커밋 낡은 전제**로 작업할 위험이 있었다. 특히 #52·#53 은 "중간지점을 하드코딩에서 실제 역으로 바꿨다"는 **구조 변경**이라 모르고 손대면 되돌릴 수 있다. PR #54 는 **머지 조건을 이미 다 충족했고 작성자 답변만 남은 상태**라, 답이 오면 다른 세션도 바로 머지할 수 있게 판단 근거를 통째로 남겼다. status.md 원칙대로 **실측한 것만 적고 미검증은 미검증이라 표기**했다(플래그 켠 상태 동작 · 카카오맵 링크 클릭) |
