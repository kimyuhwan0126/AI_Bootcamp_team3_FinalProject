...
## 🆕 팀 동시 개발 기반 (v8.1.0) — 새 저장소로 옮길 때 이 구조를 유지할 것
...
| `app/m/[code]/sections/` | 1,802줄 화면을 담당자별 파일로 분할 (→ 1,352줄) |
| PWA (`app/manifest.ts`, `public/sw.js`) | 안드로이드는 **Flutter 재작성이 아니라 TWA** (`docs/APK.md`) |

읽을 문서: `docs/팀_개발환경.md`(브랜치·저장소 설정·소유권) · `docs/APK.md`(발표용 앱)

**⚠️ 팀원 합류 전에 끝내야 하는 것** (합류 후엔 모든 브랜치가 충돌한다):
- [ ] `MeetingClient.tsx` 1,352줄 → 400줄대. STAGE MAIN/RESULT + 방장 바 분리
      (공유 상태가 많아 컨텍스트 도입 필요)
...
> ✅ v8.1.0 에서 **구조는 준비됐다.** `lib/scoring/` 에 스코어러를 파일 하나로 추가하면
