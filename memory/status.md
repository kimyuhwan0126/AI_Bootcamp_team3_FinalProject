...
### 3-3. 수단별 이동시간 (KTX·고속버스) — **원인 규명 완료, 수정 미착수**
...
#### 고쳐야 할 것 (브랜치 세션 몫 — 담당자 정해서 넘길 것)

1. **`4: "train"` 매핑 추가** — `TransitLeg["kind"]` 타입 확장 + 열차 아이콘·라벨,
   `pathType: 11` 라벨 처리
2. 🔴 **요금이 "무료"로 표시되는 버그** — `formatWon()` 이 `won <= 0` 이면 "무료"를
   돌려준다(`lib/format.ts:41`). 철도는 `fare: 0`, 대전 시외버스는 **`fare: -1`**
   (ODsay 의 "요금 정보 없음")로 오므로 **둘 다 "무료"로 뜬다.**
   4시간짜리 3회 환승 경로가 "무료"라고 표시된다 — CLAUDE.md §6 위반이다.
   ⚠️ **대전 케이스는 `verified: true` 라 진단 플래그와 무관하게 지금도 발생 중이다.**
   `-1`(정보 없음)과 `0`(진짜 무료)을 구분해야 한다
3. **두 함수의 판정 기준이 다르다 (비대칭)** — 같은 응답을 다르게 거른다:
   - `transitRouteOdsay`(**추천 계산용 이동시간**): `trafficType !== 3` 로 판정 →
     **`4` 를 탑승으로 인정해 이미 통과시키고 있다**
   - `transitRoutesDetail`(경로 상세 화면): `kind !== "walk"` 로 판정 → `4` 는 탈락
   → **추천 점수에는 이미 KTX 시간이 들어가고 있을 가능성이 높다. 미검증 — 확인 필요.**
   기준을 통일할 것
4. 열차명이 `name: ""` 로 빈다 — `lane[0].name` 이 아닌 다른 필드인 듯. 원시 응답 추가 확인 필요

**남은 관찰 도구**: `NEXT_PUBLIC_FF_ODSAY_PROBE=1`(기본 OFF)로 원시 응답을 볼 수 있다.
위 수정이 끝나면 **이 플래그는 제거 대상**이다 — 남겨두면 죽은 코드가 된다.
